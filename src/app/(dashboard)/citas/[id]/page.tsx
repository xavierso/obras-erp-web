    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <GlassCard padding="p-5" className="border-brand-blue/30">
            ...
            <p className="text-xs text-text-muted mb-4">
              Inicia la cita para registrar una visita con observaciones y evidencias para esta potencial obra.
            </p>
            {cita.obra_id ? (
              <Link href={`/obras/${cita.obra_id}/visitas/nuevo`}>
              ...
              <Button 
                ...
                onClick={async () => {
                  ...
                  try {
                    const { obrasApi } = await import('@/lib/obrasApi');
                    const nuevaObra = await obrasApi.crear(
                      cita.nombre_referencia || `Obra Potencial (Cita #${cita.id})`
                    );
                    router.push(`/obras/${nuevaObra.id}/visitas/nuevo`);
                  } catch (err: any) {
                    alert('Error al inicializar la obra potencial: ' + (err.message || 'Error desconocido'));
                    setSavingAction(false);
                  }
                }}
              >
                {savingAction ? 'Iniciando...' : 'Iniciar Cita'}
              </Button>
            )}
          </GlassCard>
