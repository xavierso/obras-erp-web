      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <Button 
            ...
            variant="outlined"
            onClick={async () => {
              if (!confirm('¿Deseas crear un nuevo presupuesto asociado a esta visita?')) return;
              try {
                setIsSaving(true);
                const { presupuestosApi } = await import('@/lib/presupuestosApi');
                const nuevo = await presupuestosApi.crear({
                  nombre: `Presupuesto Visita ${new Date(visita.fecha).toLocaleDateString()}`,
                  obra_id: obraId,
                  version: 1,
                  estado: 'borrador',
            ...
            disabled={isSaving}
            className="!min-h-[40px] px-5 py-2 flex items-center space-x-2 text-sm border-brand-blue text-brand-blue hover:bg-brand-blue/10"
          >
